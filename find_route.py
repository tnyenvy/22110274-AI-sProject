...
def find_route_on_map(start_location, end_location):
    try:
        ...
        st.write("Tọa độ điểm bắt đầu:", start_point)
        st.write("Tọa độ điểm kết thúc:", end_point)
    ...
    try:
        ...
        st.write("Số lượng nút trong đồ thị:", len(G.nodes))
    except Exception as e:
    ...
    try:
        ...
        st.write("Nút bắt đầu:", start_node)
        st.write("Nút kết thúc:", end_node)

        # Tính toán đường đi ngắn nhất
        route = nx.shortest_path(G, start_node, end_node, weight="length")
        st.write("Tuyến đường (danh sách các nút):", route)

        # Chuyển đổi đường đi thành danh sách tọa độ
        route_coords = [(G.nodes[node]['y'], G.nodes[node]['x']) for node in route]
        st.write("Danh sách tọa độ của tuyến đường:", route_coords)

        # Tạo bản đồ
        map_center = [(start_point[0] + end_point[0]) / 2, (start_point[1] + end_point[1]) / 2]
        route_map = folium.Map(location=map_center, zoom_start=12)  # Giảm zoom để phù hợp với khu vực TP.HCM
